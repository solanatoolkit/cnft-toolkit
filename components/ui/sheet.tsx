...
export interface SheetPortalProps extends SheetPrimitive.DialogPortalProps, VariantProps<typeof portalVariants> {
  className?: string;
}

const SheetPortal = ({ position, className, children, ...props }: Omit<SheetPortalProps, 'className'> & { className?: string }) => (
  <SheetPrimitive.Portal {...props}>
    <div className={cn(className, portalVariants({ position }))}>{children}</div>
...
const sheetVariants = tv({
  base: "fixed z-50 scale-100 gap-4 bg-white-diag p-6 opacity-100 shadow-dialog border",
  variants: {
    position: {
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, DialogContentProps>(
  ({ position = "left", size, className, children, ...props }, ref) => (
